...
const emptySnapshot = (): CustomerSnapshot => ({
  name: "",
  email: null,
  phone: null,
  address: null,
...
const CreateProformaPage = () => {
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  // Customer
  const [customerSnapshot, setCustomerSnapshot] =
    useState<CustomerSnapshot>(emptySnapshot());
  ...
  // Product/variant search popovers — one per line item key
  ...
  const { mutate: createProforma, isPending } = useMutation({
    mutationFn: (input: CreateProformaInput) =>
      proformaApi.createProforma(input),
    onSuccess: () => {
  ...
  const ghsRate = company?.usdToGhsRate ?? 1;
  const taxPercent = company?.taxRate ?? 0;
  ...
  const handleClearCustomer = () => {
    ...
    setCustomerSnapshot(emptySnapshot());
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: CreateProformaInput = {
      customerSnapshot,
  ...
  return (
    <>
      <PageMeta
        title={PAGE_META_DATA.proformaNew.title}
        description={PAGE_META_DATA.proformaNew.description}
      />
